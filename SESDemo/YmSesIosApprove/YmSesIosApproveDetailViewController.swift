//
// YmSesIosApproveDetailViewController.swift
// SESDemo
//
...
class YmSesIosApproveDetailViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private let logger = Logger.shared(named: "YmSesIosApproveMasterViewControllerLogger")
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "updateEntity" {
            ...
            let detailViewController = dest.viewControllers[0] as! YmSesIosApproveDetailViewController
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 17:
            ...
            cell.keyName = "HeadToItems"
            cell.value = " "
            cell.accessoryType = .disclosureIndicator
            return cell
        case 18:
            let cell = CellCreationHelper.cellForDefault(tableView: tableView, indexPath: indexPath, editingIsAllowed: false)
            cell.keyName = "YmSESIOSITEMSSet"
            cell.value = " "
            cell.accessoryType = .disclosureIndicator
            return cell
        default:
    ...
    override func tableView(_: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case 17:
            ...
            func loadProperty(_ completionHandler: @escaping ([YmSesIosItems]?, Error?) -> Void) {
                self.ymsesapprovalsrvEntities.loadProperty(YmSesIosApprove.headToItems, into: self.entity) { error in
                    self.hideFioriLoadingIndicator()
                    if let error = error {
                        completionHandler(nil, error)
                        return
                    }
                    completionHandler(self.entity.headToItems, nil)
                }
            }
            (masterViewController as! YmSesIosItemsMasterViewController).loadEntitiesBlock = loadProperty
            masterViewController.navigationItem.title = "HeadToItems"
        ...
        case 18:
            ...
            var masterViewController = destinationStoryBoard.instantiateViewController(withIdentifier: "YmSesIosItemsMaster")
            func loadProperty(_ completionHandler: @escaping ([YmSesIosItems]?, Error?) -> Void) {
                self.ymsesapprovalsrvEntities.loadProperty(YmSesIosApprove.ymSESIOSITEMSSet, into: self.entity) { error in
                    self.hideFioriLoadingIndicator()
                    if let error = error {
                        completionHandler(nil, error)
                        return
                    }
                    completionHandler(self.entity.ymSESIOSITEMSSet, nil)
                }
            }
            (masterViewController as! YmSesIosItemsMasterViewController).loadEntitiesBlock = loadProperty
            masterViewController.navigationItem.title = "YmSESIOSITEMSSet"
            (masterViewController as! YmSesIosItemsMasterViewController).preventNavigationLoop = true
            self.navigationController?.pushViewController(masterViewController, animated: true)
        default:
            return
    ...
    private func cellForAppFlag(tableView: UITableView, indexPath: IndexPath, currentEntity: YmSesIosApprove, property: Property) -> UITableViewCell {
        var value = ""
        if let propertyValue = currentEntity.appFlag {
            value = "\(propertyValue)"
        }
        return CellCreationHelper.cellForProperty(tableView: tableView, indexPath: indexPath, entity: self.entity, property: property, value: value, editingIsAllowed: self.allowsEditableCells, changeHandler: { (newValue: String) -> Bool in
            var isNewValueValid = false
...
extension YmSesIosApproveDetailViewController: EntityUpdaterDelegate {
    func entityHasChanged(_ entityValue: EntityValue?) {
        if let entity = entityValue {
            let currentEntity = entity as! YmSesIosApprove
