//
// SubscriptionMasterViewController.swift
// SESDemo
//
...
class SubscriptionMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([Subscription]?, Error?) -> Void) -> Void)?
    private var entities: [Subscription] = [Subscription]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let subscription = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "ID", value: "\(subscription.id!)")
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! SubscriptionDetailViewController
...
extension SubscriptionMasterViewController: EntitySetUpdaterDelegate {
