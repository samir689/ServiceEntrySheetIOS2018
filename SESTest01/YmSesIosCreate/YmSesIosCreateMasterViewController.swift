//
// YmSesIosCreateMasterViewController.swift
// SESTest01
//
...
class YmSesIosCreateMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([YmSesIosCreate]?, Error?) -> Void) -> Void)?
    private var entities: [YmSesIosCreate] = [YmSesIosCreate]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let ymsesioscreate = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "SheetNo", value: "\(ymsesioscreate.sheetNo!)")
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! YmSesIosCreateDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].sheetNo ?? ""
...
extension YmSesIosCreateMasterViewController: EntitySetUpdaterDelegate {
