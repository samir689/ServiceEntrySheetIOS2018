//
// YmSesIosSearhHelpMasterViewController.swift
// SESTest01
//
...
class YmSesIosSearhHelpMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([YmSesIosSearhHelp]?, Error?) -> Void) -> Void)?
    private var entities: [YmSesIosSearhHelp] = [YmSesIosSearhHelp]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let ymsesiossearhhelp = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "Ebeln", value: "\(ymsesiossearhhelp.ebeln!)")
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! YmSesIosSearhHelpDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].ebeln ?? ""
...
extension YmSesIosSearhHelpMasterViewController: EntitySetUpdaterDelegate {
