//
// NotificationMasterViewController.swift
// SESTest01
//
...
class NotificationMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([Notification]?, Error?) -> Void) -> Void)?
    private var entities: [Notification] = [Notification]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let notification = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "ID", value: "\(notification.id!)")
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! NotificationDetailViewController
...
extension NotificationMasterViewController: EntitySetUpdaterDelegate {
