...
class OnboardingManager {
    ...
    func basicAuthenticationWithOptionalValidationStep() -> BasicAuthenticationStep {
        let basicAuthenticationStep = BasicAuthenticationStep()
        basicAuthenticationStep.sendsValidationRequest = ConnectivityUtils.isConnected()
    ...
    private var restoringSteps: [OnboardingStep] {
        return [
            ...
            self.configuredWelcomeScreenStep(),
            CompositeStep(steps: SAPcpmsDefaultSteps.configuration),
            self.basicAuthenticationWithOptionalValidationStep(),
            CompositeStep(steps: SAPcpmsDefaultSteps.settingsDownload),
    ...
    /// Steps executed during Syncing.
    private var syncingSteps: [OnboardingStep] {
        return [
            self.configuredWelcomeScreenStep(),
            CompositeStep(steps: SAPcpmsDefaultSteps.settingsDownload),
            CompositeStep(steps: SAPcpmsDefaultSteps.applyDuringRestore.map { ($0 as? PasscodePolicyApplyStep)?.defaultPasscodePolicy = nil; return $0 }),
    ...
    private func configuredWelcomeScreenStep() -> WelcomeScreenStep {
        let discoveryConfigurationTransformer = DiscoveryServiceConfigurationTransformer(applicationID: "ses.xom.com", authenticationPath: "SES_Approval")
        ...
        welcomeScreenStep.welcomeScreenCustomizationHandler = { welcomeStepUI in
            welcomeStepUI.headlineLabel.text = "SESTest01"
    ...
    func restoreForSyncing() {
        self.state = .onboarding
        var context = OnboardingContext(presentationDelegate: presentationDelegate)
        context.credentialStore = self.credentialStore
        if let onboardingID = self.onboardingID {
            context.onboardingID = onboardingID
        }
        OnboardingFlowController.restore(on: self.syncingSteps, context: context, completionHandler: self.restoreCompleted)
    }
